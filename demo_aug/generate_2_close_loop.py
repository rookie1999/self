...
class CPPolicy:
    ...
    def __init__(
        ...
        # [新增] 初始化新约束标志位为 True，确保第一次运行时触发规划
        self.new_constraint = True 
        
        # [新增] 闭环控制状态变量
        self.step_in_current_constraint = 0
    ...
    def reset(self):
        ...
        # [新增] 追踪当前约束执行到了第几步
        self.step_in_current_constraint = 0 
        # [新增] 缓存选定的对称性变换矩阵，保证整个阶段的一致性
        self.cached_transforms = None
        # [新增] 重置闭环状态
        self.step_in_current_constraint = 0
        self.cached_transforms = None
        
        # [新增] 重置为 True，以便开始第一个任务的规划
        self.new_constraint = True
    ...
    def solve_single_step_closed_loop(
        self,
        constraint: Constraint,
        obs: Dict[str, Any],
        t_idx: int,
        X_transforms: Tuple[np.ndarray, np.ndarray, np.ndarray],
        fallback_to_kpts_to_robot_q: bool = True  # [新增] 添加缺少的参数，默认开启
    ) -> np.ndarray:
        """
        闭环单步规划器：结合当前观测和 Demo 的第 t_idx 帧，计算下一步动作。
        包含实时语义匹配修正 (Local Delta) 和 IK 解算。
        """
        # 1. 解包缓存的变换矩阵
        X_obj0_transf, X_obj1_transf, X_eef_transf = X_transforms
        ...
        if self.use_semantic_matching and self.sd_matcher is not None:
            # 检查数据完整性
            has_src_img = "agentview_image" in constraint.src_obs and len(constraint.src_obs["agentview_image"]) > t_idx
            has_tgt_img = "agentview_image" in obs
            has_tgt_depth = "agentview_depth" in obs
            has_cam = "camera_params" in constraint.src_obs and constraint.src_obs["camera_params"][t_idx] is not None
            ...
            if has_src_img and has_tgt_img and has_tgt_depth and has_cam:
                try:
                    ...
                    if src_img_np.ndim == 3 and src_img_np.shape[-1] >= 3:
                        ...
                        src_img = Image.fromarray(src_img_np[..., :3])
                        tgt_img = Image.fromarray(tgt_img_np[..., :3])

                        # B. 计算 Source (Demo) 在 t_idx 时的 3D 关键点
                        # 注意：这里计算的是 Demo 在该时刻的“理论位置”
                        if constraint.constraint_type == "robot-object":
                        ...
                        # D. 执行 SD 特征匹配 (Visual Matching)
                        # 在当前观测中寻找 Demo 关键点的对应位置
                        matched_points = self.sd_matcher.match_points(src_img, tgt_img, [[u_src, v_src]])
                        u_tgt, v_tgt = matched_points[0]

                        if self.verbose:
                            logging.info(f"Closed-Loop SD Matching (t={t_idx}): Src({u_src}, {v_src}) -> Tgt({u_tgt}, {v_tgt})")

                        # E. 反投影得到 Target 3D 点 (Deprojection)
                        ...
                        tgt_intrinsics = src_cam_params["intrinsics"] # 假设内参不变
                        ...
                        if z_val > 0.1 and z_val < 5.0:
                            ...
                            # 2. 临时计算几何推算位置 (用于计算差值)
                            # 这部分逻辑是从几何规划部分复用的，为了对比差异
                            if constraint.constraint_type == "robot-object":
                                task_relev_obj = constraint.obj_names[0]
                                demo_obj_geoms_size = constraint.src_obj_geoms_size[task_relev_obj][t_idx]
                                current_obj_geoms_size = obs[f"{task_relev_obj}_geoms_size"]
                                scale_factor = get_scale_factor(demo_obj_geoms_size, current_obj_geoms_size) if not self.ignore_scaling else 1.0
                                
                                P_obj_target_raw = constraint.keypoints_obj_frame[task_relev_obj][t_idx] * scale_factor
                                P_W_target_geom = transform_keypoints(
                                    P_obj_target_raw, obs[f"{task_relev_obj}_pose"] @ X_obj0_transf
                                )[:, :3][0]
                            else:
                                # object-object 暂时简化处理，假设 scale=1
                                P_obj_target_raw = np.array(constraint.keypoints_obj_frame[constraint.obj_names[1]][t_idx])
                                P_W_target_geom = transform_keypoints(
                                    P_obj_target_raw, obs[f"{constraint.obj_names[1]}_pose"] @ X_obj1_transf
                                )[:, :3][0]

                            # 3. 计算实时偏差
                            local_delta = P_W_target_semantic - P_W_target_geom
                            
                            # [安全熔断] 如果偏差过大 (例如 > 15cm)，认为匹配错误，忽略本次修正
                            if np.linalg.norm(local_delta) > 0.15:
                                logging.warning(f"Local delta too large ({np.linalg.norm(local_delta):.3f}m), ignoring.")
                                local_delta = np.zeros(3)
                            else:
                                logging.info(f"Closed-Loop Delta: {local_delta}")
                        
                        if False:  # 强制开启可视化，调试完成后可改为 if self.verbose:
                            try:
                                import matplotlib
                                ...
                                # 1. 准备保存目录
                                save_dir_root = get_save_dir()
                                if save_dir_root is None:
                                    save_dir_root = "debug_vis" # 防止路径为空
                                save_root = os.path.join(save_dir_root, "vis_closed_loop")
                                ...
                                axes[0].set_title(f"Source (Demo t={t_idx})")
                                # 画出源特征点 (红十字)
                                axes[0].plot(u_src, v_src, marker='+', color='red', markersize=15, markeredgewidth=2)
                                
                                # 右图：Target Observation
                                axes[1].imshow(tgt_img)
                                axes[1].set_title(f"Target (Step {t_idx}) Depth:{z_val:.2f}m")
                                # 画出匹配点 (绿十字)
                                axes[1].plot(u_tgt, v_tgt, marker='+', color='lime', markersize=15, markeredgewidth=2)
                                
                                # 3. 保存
                                #加上时间戳防止覆盖，t_idx 方便查看序列
                                timestamp = datetime.now().strftime("%H%M%S_%f")
                                filename = f"step_{t_idx:03d}_{timestamp}.jpg"
                                plt.tight_layout()
                                plt.savefig(os.path.join(save_root, filename), dpi=100)
                                plt.close(fig) # 极其重要：关闭图像释放内存！
                                
                                # 打印一条日志确认保存成功
                                # print(f"Saved visualization to {filename}")
                            except Exception as e_vis:
                                logging.warning(f"Visualization failed: {e_vis}")


                except Exception as e:
                    logging.warning(f"Closed-Loop Semantic matching failed: {e}")
                    local_delta = np.zeros(3) # 回退到纯几何

        # ==================== [几何规划模块] ====================
        
        # 1. 计算基础几何目标点 (P_W_target)
        if constraint.constraint_type == "robot-object":
            task_relev_obj = constraint.obj_names[0]
            link_to_kpts = {
                link: kpts[t_idx] for link, kpts in constraint.keypoints_robot_link_frame.items()
            }
            ...
            # 应用变换
            P_obj_target = constraint.keypoints_obj_frame[task_relev_obj][t_idx] * scale_factor
            P_W_target = transform_keypoints(
                P_obj_target, obs[f"{task_relev_obj}_pose"] @ X_obj0_transf
            )[:, :3]
            
        elif constraint.constraint_type == "object-object":
            # object-object 的特殊处理
            link_to_kpts = {
                link: kpts[t_idx] for link, kpts in constraint.keypoints_obj_frame.items()
                if link in constraint.obj_to_parent_attachment_frame and constraint.obj_to_parent_attachment_frame[link] is not None
            }
            P_obj_target = np.array(constraint.keypoints_obj_frame[constraint.obj_names[1]][t_idx])
            P_W_target = transform_keypoints(
        ...
        # 2. [核心修正] 将语义匹配计算出的偏差应用到目标点上
        P_W_target_corrected = P_W_target + local_delta

        # 3. 设置配置对象的关键点 (用于后续 IK)
        flip_eef = not np.allclose(X_eef_transf, np.eye(4), rtol=1e-5, atol=1e-8)
        ...
        # 4. 可视化 (可选)
        if self.viz_robot_kpts:
            set_mocap_pos_and_update_viewer(self.env, P_W_target_corrected, update_viewer=False)
            # 这里可以添加更多可视化逻辑

        # ==================== [IK 解算模块] ====================

        # 1. 计算这一步的 EEF 目标位姿
        # 注意：compute_X_W_eef_des 内部也会用到 scale_factor，这里简单调用
        # 它主要利用 eef_configuration 和 robot_configuration 的当前状态
        X_W_eef_des = compute_X_W_eef_des(
            constraint, obs, self.robot_configuration, self.eef_configuration,
            X_obj0_transf, X_obj1_transf, X_eef_transf,
            src_t_idx=t_idx, ignore_scaling=self.ignore_scaling
        )

        # 2. 更新 EEF 配置对象到目标状态
        q_eef_config = np.zeros(9) # 7 (pose) + 2 (gripper)
        ...
        # 3. 执行逆运动学 (IK) 优化
        # 使用上一帧的关节角度作为初始猜测 (Warm Start)
        prev_q = self.robot_configuration.q[self.robot_configuration.robot_idxs]
        
        # 为了更精准，这里可以使用基于关键点的优化，目标点为修正后的 P_W_target_corrected
        # 或者使用基于 EEF Pose 的优化。这里为了与 solve_constraint 保持一致，使用混合策略或直接优化 Pose。
        # 考虑到 P_W_target_corrected 包含了语义修正，我们最好利用它。
        
        # 策略 A: 仅使用 optimize_robot_configuration (基于 Pose)
        # 这种方式依赖 compute_X_W_eef_des 计算出的 X_W_eef_des 是否准确。
        # 由于 X_W_eef_des 是基于几何关系推导的，可能没有包含 local_delta。
        # 因此，我们需要手动修正 X_W_eef_des 的位置部分。
        
        # [修正 EEF 目标]：将 local_delta 加到 EEF 目标位置上
        # 注意：这假设物体偏差直接平移对应到 EEF 偏差，对于抓取任务通常成立。
        X_W_eef_des[:3, 3] += local_delta
        ...
        optimized_q = optimize_robot_configuration(
            self.robot_configuration,
            self.eef_configuration,
            retract_q=np.array([0, np.pi / 16.0, 0.00, -np.pi / 2.0 - np.pi / 3.0, 0.00, np.pi - 0.2, np.pi / 4, 0, 0]),
            update_gripper_qpos=False,
            retract_q_weights=np.array([2, 2, 1, 1, 1, 0.1, 0.1, 0, 0]),
            start_q=prev_q,
            eef_interp_mink_mp=self.eef_interp_mink_motion_planer,
            optimization_type="scipy",
            verbose=self.verbose,
        )

        if optimized_q is None:
            # 如果 IK 失败，尝试回退到基于 Keypoints 的 IK
            if fallback_to_kpts_to_robot_q:
                logging.info(f"Closed-loop IK failed, falling back to KP optimization (t={t_idx})")
                optimized_q = optimize_robot_configuration_kp(
                    self.robot_configuration,
                    P_W_target_corrected, # 使用修正后的关键点
                    prev_q,
        ...
        if optimized_q is None:
            logging.error(f"Failed to solve IK in closed-loop step t={t_idx}")
            return None
        ...
        # 从 Robot Configuration 中提取 EEF 位姿作为 Action (OSC Controller 输入)
        eef_pose_mat = self.robot_configuration.get_transform_frame_to_world(
            "gripper0_right_right_gripper", "body"
        ).as_matrix() # 这里用 "body" 还是 "site" 取决于控制器配置，通常 generate_2.py 用的是 body 配合 site offset 或者直接 site
        
        # 修正：根据 generate_2.py 的 solve_constraint 逻辑，action 使用的是 site
        eef_pose_site = self.robot_configuration.get_transform_frame_to_world(
            "gripper0_right_grip_site", "site"
        ).as_matrix()

        action = np.zeros(7)
        action[:3] = eef_pose_site[:3, 3]
        action[3:6] = quat2axisangle(R.from_matrix(eef_pose_site[:3, :3]).as_quat())
        # 夹爪动作
    ...
    def advance_constraint(self):
        ...
        # [新增] 清除缓存的对称性变换，以便在下一关重新计算最佳抓取姿态
        self.cached_transforms = None
        # [新增/关键] 标记进入了新约束，触发 get_action 中的 Motion Planning
        self.new_constraint = True
    ...
    def get_action(self, obs: Dict[str, Any]) -> Dict[str, Union[np.ndarray, str]]:
        ...
        if self.check_constraint:
            ...
            # [新增] 切换到新约束时，重置闭环追踪计数器和缓存
            # 确保下一关从头开始，并且重新计算最佳抓取姿态
            self.step_in_current_constraint = 0
            self.cached_transforms = None
        ...
        # =========================================================================
        # 阶段 A: 新任务初始化与“赶路”规划 (Motion Planning Phase)
        # =========================================================================
        if self.new_constraint:
            constraint = self.current_constraint
            
            # A.1 更新环境信息 (将物体 attach 到机器人模型上，或者更新物体位姿)
            self._update_robot_configuration(
                self.robot_configuration,
                eef_configuration=self.eef_configuration,
                env=self.env,
            ...
            # 确保 obs 里有 env 句柄，供内部可视化使用
            obs["env"] = self.env 

            # A.2 确定最佳对称性变换 (只做一次，并缓存，保证整个阶段一致性)
            if self.cached_transforms is None:
                # 返回 (X_obj0, X_obj1, X_eef)
                self.cached_transforms = find_best_transform(
                    constraint, obs, self.robot_configuration, self.eef_configuration
                )
            
            # A.3 求解约束的第一步 (t=0) 作为“赶路”的终点
            # 我们调用单步规划器来计算 t=0 时的理想构型。
            # 此时虽然 obs 是“远处”的，SD 匹配可能不准，但几何部分是准的，足以作为 motion plan 的目标。
            # 注意：solve_single_step_closed_loop 内部会自动处理语义匹配（如果有的话）
            target_action = self.solve_single_step_closed_loop(
                constraint,
                obs,
                t_idx=0,
                X_transforms=self.cached_transforms
            )
            
            if target_action is None:
                 return {"action": None, "failure_type": "failed_to_solve_initial_constraint_step"}

            # 获取计算出的机器人目标构型 (joint positions)
            # 注意：solve_single_step_closed_loop 内部已经调用了 robot_configuration.update()
            target_q = self.robot_configuration.q[self.robot_configuration.robot_idxs]
            
            # A.4 规划赶路轨迹 (Open Loop)
            # 将目标构型传给 Curobo 或其他规划器
            obs["attached_object_names"] = constraint.attached_object_names
            
            a = time.time()
            mp_result = self.motion_plan(
                obs,
                future_configurations=[target_q], # 传入目标关节角度
                future_actions=[target_action],   # 传入目标 Action (用于 eef_interp)
                motion_planner_type=self.motion_planner.name if self.motion_planner is not None else None,
            ...
            if not mp_result.success:
                logging.error("Failed to solve motion plan. Returning no action.")
                return {"action": None, "failure_type": "motion_planning_failure"}

            # A.5 将赶路动作填入队列
            self.action_queue = mp_result.actions
            ...
            # [关键改变] 我们只把 Motion Actions 放入队列，Constraint Actions 留给下面闭环生成

        # =========================================================================
        # 阶段 B: 动作执行 (Execution Phase)
        # =========================================================================
        
        # 情况 1: 队列里有动作 (正在赶路中，或者在执行 Post-Constraint Heuristics)
        # 只要队列不为空，就优先执行队列里的动作（Open Loop 执行）
        if self.action_queue:
            action = self.action_queue.pop(0)
            return {"action": action, "failure_type": None}
        
        # 情况 2: 队列空了，进入闭环精细操作阶段 (Closed-Loop Execution)
        constraint = self.current_constraint
        total_steps = len(constraint.timesteps)
        
        if self.step_in_current_constraint < total_steps:
            # >>> 闭环控制核心：每次只规划一步 <<<
            a = time.time()
            
            # B.1 更新环境配置
            # 这一步非常重要！因为机器人动了，必须更新运动学链，否则 IK 会基于旧位置解算
            self._update_robot_configuration(
                self.robot_configuration,
                eef_configuration=self.eef_configuration,
                env=self.env, 
            ...
            # B.2 闭环求解当前步
            # 传入最新的 obs 和当前的 step index
            action = self.solve_single_step_closed_loop(
                constraint, 
                obs,           # 这里的 obs 是最新的！包含了当前相机图像
                t_idx=self.step_in_current_constraint, 
                X_transforms=self.cached_transforms
            )
            
            if action is None:
                 return {"action": None, "failure_type": "closed_loop_solve_failed"}
            
            if self.verbose:
                logging.info(f"Closed-loop step {self.step_in_current_constraint} took {time.time() - a:.4f}s")

            self.step_in_current_constraint += 1
            
            # B.3 检查是否是最后一步
            if self.step_in_current_constraint >= total_steps:
                # 约束阶段结束，处理后续启发式动作 (Lift, Open Gripper 等)
                if constraint.post_constraint_behavior:
                    heuristic_actions = []
                    last_action = action
                    for behavior in constraint.post_constraint_behavior:
                        if behavior == "lift":
                            heuristic_actions.extend(generate_lift_actions(last_action))
                        elif behavior == "open_gripper":
                            heuristic_actions.extend(generate_open_gripper_actions(last_action))
                        elif behavior == "close_gripper":
                            heuristic_actions.extend(generate_close_gripper_actions(last_action))
                        if heuristic_actions:
                            last_action = heuristic_actions[-1]
                    
                    # 将 heuristic actions 加入队列，下一次循环会进入“情况 1”执行它们
                    self.action_queue = [np.array(a) for a in heuristic_actions]
                    # 暂时不标记 check_constraint，等队列跑完再标记
                else:
                    # 如果没有后续动作，直接标记完成
            ...
            # 情况 3: 所有步骤（包括闭环和启发式动作）都做完了
            self.check_constraint = True
            # 递归调用自己，触发 advance_constraint 进入下一阶段
