...
class CPPolicy:
    ...
    def solve_constraint(
        ...
        # =========================================================================
        # [修改重点] 1. 在循环外计算初始修正量 (Global Delta)
        # =========================================================================
        global_delta = np.zeros(3)
        
        if self.use_semantic_matching and self.sd_matcher is not None:
            # 强制只使用第 0 帧进行匹配，因为此时 obs 和 demo 在时序上是对齐的
            t_match_idx = 0
            
            # 安全检查：确保所有必要的数据都存在
            has_src_img = "agentview_image" in constraint.src_obs and len(constraint.src_obs["agentview_image"]) > t_match_idx
            has_tgt_img = "agentview_image" in obs
            has_tgt_depth = "agentview_depth" in obs
            has_cam = "camera_params" in constraint.src_obs and constraint.src_obs["camera_params"][t_match_idx] is not None
            
            if has_src_img and has_tgt_img and has_tgt_depth and has_cam:
                try:
                    # --- A. 准备图像 ---
                    src_img_np = constraint.src_obs["agentview_image"][t_match_idx]
                    ...
                    if src_img_np.ndim == 3 and src_img_np.shape[-1] >= 3:
                        ...
                        # --- B. 计算 Demo 在 t=0 时的 3D 关键点 ---
                        if constraint.constraint_type == "robot-object":
                            task_obj = constraint.obj_names[0]
                            kpt_local = constraint.keypoints_obj_frame[task_obj][t_match_idx][0]
                            src_pose = constraint.src_obj_pose[task_obj][t_match_idx]
                        ...
                        # --- C. 投影到 Source 图像平面 ---
                        src_cam_params = constraint.src_obs["camera_params"][t_match_idx]
                        ...
                        if self.verbose:
                            logging.info(f"Initial SD Matching (t=0): Src({u_src}, {v_src}) -> Tgt({u_tgt}, {v_tgt})")
                        ...
                        # [关键修复] 必须进行 OpenGL 到 OpenCV 的坐标系转换
                        ...
                        if z_val > 0.1 and z_val < 5.0:
                            ...
                            # 2. 几何计算出的理论目标点 (t=0)
                            # 为了计算 delta，我们需要知道“如果不加语义修正，t=0 时我们会去哪里”
                            # 复用几何计算逻辑 (假设 scale=1)
                            if constraint.constraint_type == "robot-object":
                                P_obj_target_0 = constraint.keypoints_obj_frame[constraint.obj_names[0]][t_match_idx]
                                P_W_target_geom_0 = transform_keypoints(
                                    P_obj_target_0, obs[f"{constraint.obj_names[0]}_pose"] @ X_obj0_transf
                                )[:, :3][0]
                            else:
                                P_obj_target_0 = constraint.keypoints_obj_frame[constraint.obj_names[1]][t_match_idx]
                                P_W_target_geom_0 = transform_keypoints(
                                    P_obj_target_0, obs[f"{constraint.obj_names[1]}_pose"] @ X_obj1_transf
                                )[:, :3][0]

                            # 3. 计算全局偏差
                            # Delta = 眼睛看到的真实位置 - 脑子里预想的位置
                            global_delta = P_W_target_semantic - P_W_target_geom_0
                            logging.info(f"Calculated Global Delta: {global_delta}")

                            # --- 可视化 (仅 t=0) ---
                            # 这里可以保留你的 vis 代码，但只会执行一次，生成 match_step_000_xxx.jpg
                            if False:
                                try:
                                    import matplotlib
                                    matplotlib.use('Agg')
                                    import matplotlib.pyplot as plt
                                    from demo_aug.generate_2 import get_save_dir # 确保能获取到保存路径

                                    save_root = os.path.join(get_save_dir(), "vis")
                                    os.makedirs(save_root, exist_ok=True)
                                    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
                                    axes[0].imshow(src_img)
                                    axes[0].set_title(f"Source (t={t_match_idx})")
                                    axes[0].plot(u_src, v_src, marker='+', color='red', markersize=15, markeredgewidth=2)
                                    axes[1].imshow(tgt_img)
                                    axes[1].set_title(f"Target (Depth: {z_val:.2f}m)")
                                    axes[1].plot(u_tgt, v_tgt, marker='+', color='lime', markersize=15, markeredgewidth=2)
                                    timestamp = datetime.now().strftime("%H%M%S_%f")
                                    filename = f"match_initial_{timestamp}.jpg"
                                    plt.tight_layout()
                    ...
                    logging.warning(f"Initial Semantic matching failed: {e}")
                    # 失败了 global_delta 保持为 0，回退到纯几何方法
        # =========================================================================
        ...
        for t_idx in range(solve_first_n_steps):
            ...
            # =========================================================================
            # [修改重点] 2. 将计算好的全局偏差应用到每一步的目标点上
            # =========================================================================
            # 无论这是第几步，都加上开始前算好的那个固定偏移量
            # 如果匹配失败 global_delta 为 0，不影响结果
            P_W_target = P_W_target + global_delta
            # =========================================================================
