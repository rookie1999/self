...
# pydrake.multibody._mesh_model_maker.py
...
# taken from manipulation/meshcat_utils.py
...
class MeshModelMaker:
    ...
    def __init__(
        ...
        *,
        ...
        if output_dir is None:
            ...
            raise ValueError(
                f"The output directory must name an existing directory; given {output_dir}."
            )
    ...
    def make_model(self):
        ...
        mesh_uri = MeshModelMaker._make_mesh_uri(self.encoded_package, self.mesh_path)

        if self.scale <= 0:
            raise ValueError(f"Scale value must be positive, given {self.scale}.")
        ...
        _logger.info(
            f"    Bounding box (in geometry frame): {size[0]} x {size[1]} x {size[2]} meters"
        )
    ...
    def _make_mesh_uri(package_spec: str, mesh_path: Path):
        ...
        if package_spec.endswith("package.xml"):
            ...
            if not package_spec.exists():
                raise ValueError(
                    f"The indicated package.xml cannot be found: '{package_spec}'"
                )
        ...
        if package_spec == "auto":
            ...
            if candidate_package_path.exists():
                package_name = MeshModelMaker._read_package_name(candidate_package_path)
            else:
    ...
    def _read_package_name(package_path: Path):
        ...
        if name_element is None:
            raise ValueError(
                f"The provided package.xml is missing a 'name' element: {package_path}."
            )
        return name_element.text.strip()


# no longer needed: halfspace (floor asset) was causing collision checking failuress
# solution: either remove floor asset or decrease MinimumCollisionConstraint's threshold
def find_halfspace(scene_graph):
    for geometry_id in scene_graph.model_inspector().GetAllGeometryIds():
        geometry_instance = scene_graph.model_inspector().GetShape(geometry_id)
        if isinstance(geometry_instance, HalfSpace):
            geometry_name = scene_graph.model_inspector().GetName(geometry_id)
            print(
                f"Geometry with ID {geometry_id} and name '{geometry_name}' is a HalfSpace."
            )


# Modify the SDF_TEMPLATE to include multiple visual and collision tags
_MULTIMESH_SDF_TEMPLATE = """<?xml version='1.0'?>
<sdf xmlns:drake="http://drake.mit.edu" version="1.8">
  <model name='{name}'>
    <link name='{name}'>
      <inertial>
...
class MultiMeshModelMaker(MeshModelMaker):
    def __init__(
        self,
        *,
        ...
        **kwargs,
        ...
        """
        Extends MeshModelMaker to support multiple .obj files for visual and
        collision geometries.
        """
        super().__init__(**kwargs)
        self.visual_mesh_paths = visual_mesh_paths
        self.collision_mesh_paths = collision_mesh_paths

    def _generate_geometry_xml(self, mesh_paths, geometry_type):
        """
        Helper function to generate a string containing the XML for multiple visual
        or collision geometries.
        """
        assert geometry_type in ["visual", "collision"]

        geometry_template = f"""
      <{geometry_type} name='{geometry_type}_{{i}}'>
        <pose>{{geometry_position}} 0 0 0</pose>
        <geometry>
          <mesh>
            <uri>{{mesh_uri}}</uri>
...
"""
        ...
        for i, mesh_path in enumerate(mesh_paths):
            subs = copy.deepcopy(self.subs)  # Use the attributes from parent class
            subs["i"] = i + 1  # Index for naming
            subs["mesh_uri"] = self._make_mesh_uri(self.encoded_package, mesh_path)
            geometry_xmls.append(geometry_template.format(**subs))

        return "\n".join(geometry_xmls)

    def make_model(self, filename_prefix: str = ""):
        """
        Creates a model from the mesh given at visual and collision mesh paths
        and writes the corresponding model to the file based on the maker's configurations.
        """
        super().make_model()  # Generate subs dictionary and other preparations

        visual_xml = self._generate_geometry_xml(self.visual_mesh_paths, "visual")
        collision_xml = self._generate_geometry_xml(
            self.collision_mesh_paths, "collision"
        )

        model_content = _MULTIMESH_SDF_TEMPLATE.format(
            visuals=visual_xml,
            collisions=collision_xml,
            **self.subs,  # Attributes from parent class
        )

        output_path = (
            self.output_dir
            / f"{filename_prefix}{self.mesh_path.stem}{'-multi' if len(self.collision_mesh_paths) > 0 else ''}.sdf"
        )
...
def get_collision_geometry_ids_by_model_instance_index(
    plant: MultibodyPlant, model_instance_index: ModelInstanceIndex
) -> List[GeometryId]:
    collision_geometry_ids = []
    for body_index in plant.GetBodyIndices(model_instance_index):
        body = plant.get_body(body_index)
        geometry_ids = plant.GetCollisionGeometriesForBody(body)
        collision_geometry_ids.extend(geometry_ids)
    return collision_geometry_ids
