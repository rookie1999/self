...
class Cameras(TensorDataclass):
    ...
    def _init_get_height_width(
        self,
        h_w: Union[
            TensorType["batch_hws":..., 1], TensorType["batch_hws":...], int, None
        ],
    ...
    def _generate_rays_from_coords(
        ...
        if CameraType.FISHEYE.value in cam_types:
            mask = (self.camera_type[true_indices] == CameraType.FISHEYE.value).squeeze(
                -1
            )  # (num_rays)
            mask = torch.stack([mask, mask, mask], dim=0)
    ...
    def rescale_output_resolution(
        ...
        scaling_factor: Union[
            TensorType["num_cameras":...], TensorType["num_cameras":..., 1], float, int
        ],
    ) -> None:
