...
Example usage below:
    ...
    # example usage with robomimic dataset
    PYTHONPATH=. python scripts/robomimic_playback.py --use-actions --video_skip 1 --render_image_names agentview robot0_eye_in_hand \
        --video_path test-playback-1.mp4 --dataset /scr/thankyou/autom/diffusion_policy/data/robomimic/datasets/can/ph/1_demo.hdf5
...
def playback_trajectory_with_env(
    ...
    Args:
        ...
        video_skip (int): determines rate at which environment frames are written to viqeo
        ...
        if write_video:
            if video_count % video_skip == 0:
                ...
                for cam_name in camera_names:
                    ...
                    # add text to image specifying demo name
                    if demo_name is not None:
                        # import ipdb; ipdb.set_trace()
                        img = Image.fromarray(image[::-1])
                        draw = ImageDraw.Draw(img)
                        draw.text((10, 10), demo_name)
                        draw.text((200, 0), f"action: {i}")
...
def playback_dataset(args):
    ...
    if not args.use_obs:
        ...
        env_meta["env_kwargs"]["control_freq"] = 15
        # import ipdb; ipdb.set_trace()
        ...
        if is_robosuite_env:
            ...
            if args.transform:
                ...
                if "lift" in args.dataset:
                    ...
                    obj_to_xml_body_name = {"can": "Can_main"}
                else:
                    raise ValueError(
                ...
                objs_transf_params_seq = f[
                    f"data/{ep}/timestep_0/objs_transf_params_seq"
                ]
                ...
                xml = update_model_xml(
                    ...
                    obj_to_transf_type_seq,
                    obj_to_transf_params_seq,
                    remove_body_free_joint=False,  # if remove, the object is fixed to the world
                    apply_all_transforms=True,
                    set_obj_collision_free=False,  # for pure rendering purposes, disable collisions
                    recenter_vertices=False,
                )

                initial_state["states"][10:13] = [0, 0, 0]
                initial_state["states"][13:17] = [1, 0, 0, 0]
                initial_state["model"] = xml
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transform",
        action="store_true",
        help="apply object transformations to model xml",
    )
