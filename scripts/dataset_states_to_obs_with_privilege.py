...
def dataset_states_to_obs(args):
    ...
    for ind in tqdm(range(len(demos))):
        ...
        # --------------------------------------------------------------------------
        # [新增逻辑]：从源文件搬运特权信息 (Privileged Info)
        # --------------------------------------------------------------------------
        privileged_keys = [
            "privileged_eef_pos",
        ...
        # 源文件中的 obs group
        if "data/{}/obs".format(ep) in f:
            source_obs_grp = f["data/{}/obs".format(ep)]

            for p_key in privileged_keys:
                if p_key in source_obs_grp:
                    # 读取数据
                    p_data = source_obs_grp[p_key][()]

                    # 写入到新文件的 obs 组中
                    # 注意：ep_data_grp 已经在上面创建了，且 obs 子组也因为写入了图像而自动存在
                    if args.compress:
                        ep_data_grp.create_dataset("obs/{}".format(p_key), data=p_data.astype(np.float32),
                                                   compression="gzip")
                    else:
                        ep_data_grp.create_dataset("obs/{}".format(p_key), data=p_data.astype(np.float32))

                    # (可选) 如果还需要 next_obs 的特权信息，通常 BC 不需要，这里暂不复制以节省空间
        # --------------------------------------------------------------------------
